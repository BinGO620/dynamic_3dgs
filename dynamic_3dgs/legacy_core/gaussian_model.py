...
from dynamic_3dgs.legacy_core.utils.general_utils import (
    build_rotation,
...
from dynamic_3dgs.legacy_core.utils.graphics_utils import BasicPointCloud, getWorld2View2
from dynamic_3dgs.legacy_core.utils.sh_utils import RGB2SH
...
class GaussianModel:
    ...
    def create_pcd_from_image(self, cam_info, init=False, scale=2.0, depthmap=None):
        cam = cam_info
        # adapted: images are CPU-stored (6GB policy), exposure params on CUDA
        image_ab = (
            (torch.exp(cam.exposure_a)) * cam.original_image.to(cam.exposure_a.device)
            + cam.exposure_b
        )
