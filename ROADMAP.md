...
| A7 | 2026-09-09 | legacy_core（Phase A）离线协议：keyframe 密度放开 kf_interval 5→1（重叠规则 kf_overlap/kf_cutoff/kf_translation 不变，其余超参全部 stock MonoGS）。R1 判决 A1 ✅（TUM 16.03≥15）/ A2、A3 ❌（Bonn 16.6/16.0<19），keyframe 稀疏（14-19 个/序列）为 stock 在线 SLAM 策略在离线语境的覆盖不足 | results/legacy_core/evidence/verdict_r1.md |
