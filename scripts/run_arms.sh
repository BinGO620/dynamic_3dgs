...
EXP=lifecycle_gate
ARMS=(${@:-"C0 P1 Ma Mb"})   # queue split across GPUs by arm name
STREAM=${STREAM:-"--stream-pass --stream-k 40"}
declare -A LCS=( [C0]=off [P1]=placebo [Ma]=retire [Mb]=full )
...
run_one() {
    ...
    "$PY" scripts/run_baseline.py --config "configs/bonn/${seq}.yaml" \
        --exp-id "${EXP}_${arm}" --seed 0 --lifecycle "$lc" $STREAM \
